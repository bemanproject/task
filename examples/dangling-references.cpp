...
namespace {
ex::task<int, ex::empty_env>  do_work(std::string value) { /* work */ co_return 0; };
ex::task<void, ex::empty_env> execute_all() {
    co_await ex::when_all(do_work("arguments 2"));
    co_return;
}
...
int main() {
    ...
    ex::sync_wait([]() -> ex::task<void, ex::empty_env> {
        auto t = [](const int& v) -> ex::task<int, ex::empty_env> { co_return v; }(42);
